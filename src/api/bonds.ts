...
export const getTransactions = async (): Promise<TTransaction[]> => {
  ...
  if (!res.ok) throw new Error(await parseError(res, 'Failed to fetch transactions'))
  return res.json()
}

export const getPrizes = async (): Promise<TPrize[]> => {
  const res = await fetch(`${API_BASE}/prizes`)
  if (!res.ok) throw new Error(await parseError(res, 'Failed to fetch prizes'))
  return res.json()
}
...
export const addTransaction = async (data: TNewTransaction): Promise<void> => {
  ...
  if (!res.ok) throw new Error(await parseError(res, 'Failed to add transaction'))
}
...
export const addPrize = async (data: TNewPrize): Promise<void> => {
  ...
  if (!res.ok) throw new Error(await parseError(res, 'Failed to add prize'))
}
...
export const updateTransaction = async (id: string, data: TNewTransaction): Promise<void> => {
  ...
  if (!res.ok) throw new Error(await parseError(res, 'Failed to update transaction'))
}

export const deleteTransaction = async (id: string): Promise<void> => {
  const res = await fetch(`${API_BASE}/transactions/${id}`, { method: 'DELETE' })
  if (!res.ok) throw new Error(await parseError(res, 'Failed to delete transaction'))
}
...
export const updatePrize = async (id: string, data: TNewPrize): Promise<void> => {
  ...
  if (!res.ok) throw new Error(await parseError(res, 'Failed to update prize'))
}

export const deletePrize = async (id: string): Promise<void> => {
  const res = await fetch(`${API_BASE}/prizes/${id}`, { method: 'DELETE' })
  if (!res.ok) throw new Error(await parseError(res, 'Failed to delete prize'))
}

export const calculate = async (): Promise<TResults> => {
  const res = await fetch(`${API_BASE}/calculate`)
  if (!res.ok) throw new Error(await parseError(res, 'Failed to calculate'))
  return res.json()
}
